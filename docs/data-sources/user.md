...
resource "ad_group_membership" "manager_membership" {
  ...
  members  = [data.ad_user.dept_manager.dn]
}
```
...
locals {
  direct_reports = [
    ...
    user if user.manager == data.ad_user.manager.dn
  ]
}
...
resource "ad_group_membership" "conditional_access" {
  ...
  members  = [data.ad_user.privileged_user.dn]
}
...
locals {
  admin_user = {
    dn         = data.ad_user.frequently_referenced.dn
...
resource "ad_group_membership" "conditional" {
  ...
  members  = [local.optional_user.dn]
}
```
...
2. **Multiple Lookup Methods**
   ...
   ```
   Use only one of: `id`, `dn`, `upn`, or `sam_account_name`.
...
output "debug_user_info" {
  value = {
    ...
    dn                  = data.ad_user.debug_user.dn
    upn                 = data.ad_user.debug_user.upn
